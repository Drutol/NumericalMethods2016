...
namespace NumMethods6.MathCore
{
    public static class DifferentialService
    {
        ...
        //final
        private static readonly double[] finalWeights = {0.17476028, -0.55148053, 1.20553547, 0.17118478};
        //step
        private static readonly double[] stepWeights = {0, 0.4, 0.45573726, 1};
        //weight per k value
        private static readonly double[] b2 = {0.4};
        private static readonly double[] b3 = {0.29697760, 0.15875966};
        private static readonly double[] b4 = {0.21810038, -3.05096470, 3.83286432};

        public static IEnumerable<double> Rk4(double x, IEnumerable<double> y, double step, List<DynamicDiffFun> f,
        ...
        public static IEnumerable<double> Ralston(double x, IEnumerable<double> y, double step, List<DynamicDiffFun> f,
            List<string> parameters)
        {
            var k1 = f.Select(fun => step*(double) fun(x, GetDynamicVariables(parameters, y))).ToList();

            var weightedStep = stepWeights[1]*step;
        ...
        private static dynamic GetDynamicVariables(List<string> parameters, IEnumerable<double> inits, List<double> kn,
            double dx)
        {
            var dyn = new ExpandoObject() as IDictionary<string, object>; //dynamic object Implements IDictionary
            var i = 0;
            foreach (var init in inits.Zip<double, string, Tuple<double, string>>(parameters, Tuple.Create))
